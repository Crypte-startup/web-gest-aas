...
const FactureList = () => {
  ...
  const handleExportPDF = async (facture: Facture) => {
    ...
    content.innerHTML = `
      ...
      <h1 style="color: #333; text-align: center; margin: 20px 0;">FACTURE</h1>
      <div style="margin: 20px 0;">
        <p><span style="font-weight: bold;">Client:</span> ${facture.client_name}</p>
        ${facture.motif ? `<p><span style="font-weight: bold;">Motif:</span> ${facture.motif}</p>` : ''}
        <p><span style="font-weight: bold;">Date:</span> ${new Date(facture.created_at).toLocaleDateString()}</p>
      </div>
      <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
        <thead>
          <tr>
            ...
            <th style="background-color: #f2f2f2; border: 1px solid #ddd; padding: 8px; text-align: right;">Prix unitaire (${facture.devise})</th>
            <th style="background-color: #f2f2f2; border: 1px solid #ddd; padding: 8px; text-align: right;">Prix total (${facture.devise})</th>
          </tr>
        </thead>
        <tbody>
          ${itemsHtml}
          <tr style="font-weight: bold; background-color: #f9f9f9;">
            ...
            <td style="border: 1px solid #ddd; padding: 8px; text-align: right;">${facture.montant.toLocaleString()} ${facture.devise}</td>
          </tr>
        </tbody>
      </table>
    ...
    try {
      ...
      pdf.save(`facture-${facture.client_name}-${new Date(facture.created_at).toLocaleDateString()}.pdf`);
